...
module.exports = function (req, res, next) {
    try {
        ...
        if (!authorizationHeader) {
            return next(ApiError.UnauthorizedError('Ошибка токена авторизации'));
        }
        if (authorizationHeader === 'null'|| 'undefined')   {
            return next(ApiError.UnauthorizedError('Не корректные данные авторизации'));
        }
        ...
        const userData = tokenServicervice.validateAccessToken(accessToken);
        if (!userData) {
            return next(ApiError.UnauthorizedError('overdue token'));
        }

        req.userData = userData;
        next();
    } catch (e) {
        return next(ApiError.UnauthorizedError('Fatal auth error'));
    }
};
